 ...
 * navigateur.
 *
 * Ce fichier a un jumeau assume dans frontend/src/services/chatSocket.ts.
 * L'app web historique n'est pas dans les workspaces et ne peut pas resoudre
 * ce paquet ; l'y brancher tirerait le singleton axios d'ici dans une page qui
 * a deja le sien, soit deux rafraichissements de jeton en course. Le contrat
 * est epingle cote serveur (backend/test/chatSocket.test.ts), pas par la
 * discipline.
 */
...
export const createChatSocket = (options: {
  ...
  const getToken = options.getToken ?? (() => sessionStorage.getItem(ACCESS_TOKEN_KEY));
